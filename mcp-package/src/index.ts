...
server.tool(
  ...
  {
    destination: z.string().describe("Where to travel, e.g. 'Goa', 'Paris', 'Bali'"),
    leaveDays: z.number().describe("Number of leave/vacation days available"),
    departure: z.string().describe("City flying from, e.g. 'Mumbai', 'Delhi'"),
    budget: z.string().optional().describe("Approximate budget e.g. '50000', 'flexible'"),
  },
...
Rules:
- itinerary type: travel | explore | relax | adventure
...
- All prices in INR, dates after ${today}
...
• Itinerary: ${t.itinerary.map((d: any) => `Day ${d.day}: ${d.title}`).join(" → ")}
    ...
    return {
      content: [
        {
          type: "text",
          text: `🌍 Wonderly Trip Plans for ${destination}\nFrom ${departure} | ${leaveDays} leave days | Budget: ${budget || "flexible"}\n\n${summary}`,
        },
      ],
    };
  }
